...
function isUserAuthenticated(req, res, next) {
  if (req.session.userAuthenticated) { // Check if the user is logged in
      return next();
  }
...
router.get('/', isUserAuthenticated, async (req, res) => {
  try {
    ...
    res.render('home', { user: req.session.user, layout: false, products }); // Pass user data and products to the home page
  } catch (error) {
    console.error("Error fetching products:", error.message);
    res.status(500).send("Server Error");
  }
...
router.post("/signup", async (req, res) => {
  ...
  try {
    ...
    res.render("signup", { error: "Email already exists or invalid data" });
  }
});
router.get("/logout", (req, res) => {
  req.session.destroy(() => {
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (user && (await bcrypt.compare(password, user.password))) {
      ...
      req.session.userAuthenticated = true; // Set userAuthenticated to true
...
router.post("/wishlist/add", isUserAuthenticated, async (req, res) => {
  ...
  try {
    ...
    res.redirect("/wishlist"); // Redirect to the home page or wherever you want
  } catch (error) {
...
router.get("/wishlist", isUserAuthenticated, async (req, res) => {
  const userId = req.session.user._id; // Get the user ID from the session
  ...
  try {
    ...
    res.render("wishlist", { layout: false, wishlist });
  } catch (error) {
...
router.post("/wishlist/remove", async (req, res) => {
  try {
    const { id: productId } = req.body; // Get the product ID from the request body
    const userId = req.session.user._id; // Get the user ID from the session
    ...
    await Wishlist.findOneAndUpdate(
      { user: userId },
      { $pull: { products: { product: productId } } } // Correctly reference the product field
    );
